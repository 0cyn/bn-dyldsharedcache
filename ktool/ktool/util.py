    ...
    def ansi_to_attrstr(ansi_str):
        ...
        while pos < len(ansi_str):
            if ansi_str[pos] == chr(27):
                ...
                is_reset = False
                first_item = ansi_list[0][1:]

                if first_item == '39':
                    is_reset = True
                elif 30 <= int(first_item) <= 37:
                    attr_color = int(first_item)

                if is_reset:
                    attr_end = bland_pos
                    attr_str.set_attr(attr_start, attr_end, str(attr_color))
                else:
                    attr_start = bland_pos
                    attr_str.set_attr(attr_end, attr_start, '0')
    ...
    def render(self, _rows, width, row_start):
        ...
        for row_i, row in enumerate(_rows):
            ...
            for col_i, col in enumerate(row):
                if chr(27) in col:
                    col = AttributedString.ansi_to_attrstr(col)
                    print(str(col))
                lines = []
                ...
                if isinstance(col, AttributedString):
                    print(col.attrs)
                    string_cursor = 0
                    rehighlighted_lines = []
                    for line in lines:
